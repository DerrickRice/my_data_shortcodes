...
class MenuRenderer {
  ...
  private function form_fields ($entry) {
    ...
    foreach ($this->get_schema()['schema'] as $field => $finfo) {
      ...
      } elseif ($finfo['input'] == 'number') {
        $html .= $this->number_input(
          $field,
          $entry['id'] . '-' . $field,
          isset($entry[$field]) ? $entry[$field] : '',
          $finfo['print_name'],
      ...
      } elseif ($finfo['input'] == 'checkbox') {
        $html .= $this->checkbox_input(
          $field,
          $entry['id'] . '-' . $field,
          isset($entry[$field]) ? (bool)$entry[$field] : false,
          $finfo['print_name'],
          $finfo['attrs']
        );
      } else {
  ...
  private function number_input ($field, $input_id, $value, $label, $extra_attr) {
    $html = HtmlGen::elem('div', array(
      'class' => 'mdsc_admin_field',
      'data-field' => $field
    ));
